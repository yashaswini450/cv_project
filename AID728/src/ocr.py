...
class LicensePlateOCR:
    ...
    def _run_ocr(self, plate_img: np.ndarray) -> Tuple[str, float]:
        ...
        variants = self._ocr_variants(plate_img)
        best_text, best_score = "", 0.0

        for variant in variants:
            if self._ocr_backend == "easyocr":
                text, score = self._easyocr_read(variant)
            elif self._ocr_backend == "paddle":
                text, score = self._paddle_read(variant)
            else:
                text, score = "", 0.0

            normalized = normalize_plate_text(text)
            candidate_score = score * self._plate_text_score(normalized)
            if candidate_score > best_score:
                best_text, best_score = normalized, candidate_score

        return best_text, best_score

    @staticmethod
    def _ocr_variants(plate_img: np.ndarray) -> List[np.ndarray]:
        """Generate a small set of OCR-friendly plate crops."""
        variants: List[np.ndarray] = []

        for target_w in (240, 320, 420):
            scale = target_w / (plate_img.shape[1] + 1e-6)
            new_h = max(24, int(plate_img.shape[0] * scale))
            resized = cv2.resize(
                plate_img,
                (target_w, new_h),
        ...
        # Preserve the original project preprocessing as one candidate too.
        variants.append(enhance_for_ocr(plate_img))
        return variants

    @staticmethod
    def _plate_text_score(text: str) -> float:
        """
        Score OCR candidates by how plausible they are as a license plate.
        This keeps noisy full-ROI reads from beating plate-like strings.
        """
        if not text:
            return 0.0
        ...
        if 8 <= n <= 10:
            score += 0.20
        if re.match(r"^[A-Z]{2}[0-9]{2}[A-Z]{1,3}[0-9]{3,4}$", text):
            score += 0.50
